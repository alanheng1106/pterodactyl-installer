...
export GITHUB_BASE_URL=${GITHUB_BASE_URL:-"https://raw.githubusercontent.com/alanheng1106/pterodactyl-installer"}
...
welcome() {
  ...
  if [ "$1" == "panel" ]; then
    output "Latest pterodactyl-china/panel is $PTERODACTYL_PANEL_VERSION"
  elif [ "$1" == "wings" ]; then
    output "Latest pterodactyl-china/wings is $PTERODACTYL_WINGS_VERSION"
  fi
  print_brake 70
}
